...
export default function PostForm({onSaved,onCancel}){
  ...
  const useTemplate=tId=>{const t=templates.find(x=>x.id==tId);if(t)setPost({...post,content_type:t.content_type,channel:t.channel,platform:t.platform})};
  const saveTemplate=()=>{const name=window.prompt('Template name');if(!name)return;api.post('/templates',{name,content_type:post.content_type,channel:post.channel,platform:post.platform}).then(r=>setTemplates([r.data,...templates])).catch(()=>{})};
  const onPasteLink=e=>{const pasted=(e.clipboardData||window.clipboardData).getData('text');if(!pasted)return;const u=pasted.toLowerCase();const h=u.includes('youtube.com')||u.includes('youtu.be')?{channel:'YouTube',platform:'Google'}:u.includes('instagram.com')?{channel:'Instagram',platform:'Meta'}:u.includes('tiktok.com')?{channel:'TikTok',platform:'TikTok'}:null;if(h&&(!post.channel||!post.platform)){setPost({...post,uploaded_link:pasted,channel:post.channel||h.channel,platform:post.platform||h.platform});setAutoFillHint(`Auto-filled Channel=${h.channel}, Platform=${h.platform} from link`);setTimeout(()=>setAutoFillHint(null),6000)}};
  ...
  const maybeShowPanelForName=name=>{const key=(name||'').trim().toLowerCase();const found=suggestionsCache.current[key]||suggestions.find(x=>x.project_name?.toLowerCase()===key);if(found){setSelectedProjectPanel(found);setShowSuggestions(false);setPanelOpen(true)}};
  ...
  return <div className="postform">
    ...
    <div className="form-grid">
      ...
      <select className="post-select" value={post.content_type||''} onChange={e=>setPost({...post,content_type:e.target.value})} disabled={loadingSettings}><option value="">-- Content Type --</option>{contentTypes.map((s,i)=><option key={`${s}-${i}`} value={s}>{s}</option>)}</select>
      ...
      <select className="post-select" value={post.channel||''} onChange={e=>setPost({...post,channel:e.target.value})} disabled={loadingSettings}><option value="">-- Channel --</option>{channels.map((s,i)=><option key={`${s}-${i}`} value={s}>{s}</option>)}</select>
      <select className="post-select" value={post.platform||''} onChange={e=>setPost({...post,platform:e.target.value})} disabled={loadingSettings}><option value="">-- Platform --</option>{platforms.map((s,i)=><option key={`${s}-${i}`} value={s}>{s}</option>)}</select>
      <select className="post-select" value={post.status||''} onChange={e=>setPost({...post,status:e.target.value})}><option value="Listed">Listed</option><option value="Scheduled">Scheduled</option><option value="Uploaded">Uploaded</option></select>
      <div className="schedule-field"><label htmlFor="schedule-datetime">Schedule Date &amp; Time</label><input className="post-input" id="schedule-datetime" type="datetime-local" value={post.scheduled_at||''} onChange={e=>setPost({...post,scheduled_at:e.target.value})}/></div>
      <div className="uploaded-link-field"><input className="post-input" placeholder="Uploaded Link" value={post.uploaded_link||''} onChange={e=>setPost({...post,uploaded_link:e.target.value})} onPaste={onPasteLink}/><LinkActions url={post.uploaded_link}/></div>
      ...
      <textarea className="notes-field post-input" placeholder="Notes" value={post.notes||''} onChange={e=>setPost({...post,notes:e.target.value})}/>
    </div>
