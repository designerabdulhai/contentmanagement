...
function ChannelSelect({value, onChange}){
  return <select className={`content-channel-select channel-${String(value || 'empty').toLowerCase()}`} value={value || ''} onChange={e=>onChange(e.target.value)}>
...
function ChannelBadge({value}){
  return <span className={`content-channel-badge channel-${String(value || 'empty').toLowerCase()}`}>{value || 'Not set'}</span>
}
  ...
  return <div className="page content-page">
    ...
    <div className="table-wrap card content-table-wrap">
      ...
      <table className="content-table"><thead><tr>
        <th>Name</th><th>Channel</th><th>Full Video</th><th>Short Ex</th><th>Short Top</th><th>Style Ex</th><th>Style Top</th><th>Poster</th><th>Actions</th>
      </tr></thead><tbody>{visible.map(item=><tr key={item.id}>
        <td className="content-name">{item.name}</td>
        <td><ChannelBadge value={item.channel} /></td>
    ...
    <div className="modal content-modal" role="dialog" aria-modal="true">
      ...
      <form onSubmit={submit}>
        <div className="content-form-top-grid">
          ...
          <label className="content-field"><span>Channel</span><ChannelSelect value={form.channel} onChange={value=>set('channel',value)} /></label>
        </div>
        ...
        <div className="content-modal-actions"><button className="btn-secondary" type="button" onClick={onCancel}>Cancel</button><button className="btn-primary" type="submit" disabled={saving || !form.name.trim() || !form.channel}>{saving?'Saving…':initial.id?'Save Changes':'Add Content'}</button></div>
      </form>
    </div>
  </div>
}
