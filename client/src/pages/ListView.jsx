...
const CONTENT_TYPES = ['Full Video','Short Ex','Short Top','Short Style','Short Top Style','Short Music','Desk Short','Client','Book'];
const TOKEN_KEY = 'content_schedule_auth_token';
const POSTS_CACHE_KEY = 'content_schedule_posts_cache';
...
function normalizeContentType(value){return String(value||'').trim().toLowerCase();}
...
function channelRowStyle(channel){
  const key=channelKey(channel);
  if(key==='hhd') return {background:'rgba(37,99,235,.015)'};
  if(key==='bhd') return {background:'rgba(139,0,0,.015)'};
  return undefined;
}

export default function ListView(){
  const [posts,setPosts]=useState(readCachedPosts);
  ...
  const load=()=>api.get('/posts').then(r=>{
    const next=Array.isArray(r.data)?r.data:[];
    setPosts(next);
    try{localStorage.setItem(POSTS_CACHE_KEY,JSON.stringify(next))}catch{}
    return next;
  }).catch(()=>null).finally(()=>setPostsLoading(false));
  ...
  useEffect(()=>{
    const onPointerDown=(event)=>{
      if(contentTypeRef.current && !contentTypeRef.current.contains(event.target)) setContentTypeOpen(false);
    };
  ...
  const toggleContentType=(type)=>{
    setSelectedContentTypes(current=>current.includes(type)?current.filter(item=>item!==type):[...current,type]);
  };
  const clearContentTypes=()=>setSelectedContentTypes([]);
  ...
  const deletePost=async(id)=>{
    ...
    try{
      ...
      setPosts(current=>{
        const next=current.filter(post=>Number(post.id)!==Number(id));
        try{localStorage.setItem(POSTS_CACHE_KEY,JSON.stringify(next))}catch{}
        return next;
      });
    }catch(error){
  ...
  const filteredPosts=useMemo(()=>{
    ...
    const selectedTypeKeys=selectedContentTypes.map(normalizeContentType);
    ...
    return posts.filter(p=>{
      ...
      const matchesContentType=selectedTypeKeys.length===0 || selectedTypeKeys.includes(normalizeContentType(p.content_type));
      return (!filters.search||(p.project_name||'').toLowerCase().includes(filters.search.toLowerCase()))
  ...
  },[posts,filters,myPostsOnly,selectedContentTypes]);
  ...
  const changeStatus=async(post,status)=>{
    ...
    try{
      ...
      setPosts(current=>{
        const next=current.map(item=>item.id===post.id?updated:item);
        try{localStorage.setItem(POSTS_CACHE_KEY,JSON.stringify(next))}catch{}
        return next;
      });
    }catch(e){}finally{setUpdatingStatus(null)}
  };

  const actionButtons=p=><td className="actions">
  ...
  const tableRows=items=>items.map(p=><tr key={p.id} className={`channel-row channel-row-${channelKey(p.channel).replace(/[^a-z0-9]+/g,'-')}`} style={channelRowStyle(p.channel)}>
    ...
    <td><LinkActions url={p.uploaded_link}/></td>
    {actionButtons(p)}
  </tr>);

  const contentTypeLabel=selectedContentTypes.length===0?'Content Type':selectedContentTypes.length===1?selectedContentTypes[0]:`${selectedContentTypes.length} Content Types`;
  const loadingSkeleton=!posts.length&&postsLoading;

  return <div className="page listview">
    <div className="list-header">
      ...
      <div className="list-actions">
        ...
        <div ref={contentTypeRef} style={{position:'relative'}}>
          <button
            className={`list-tool-btn ${selectedContentTypes.length?'active':''}`}
            type="button"
          ...
          {contentTypeOpen&&<div role="menu" style={{position:'absolute',right:0,top:'calc(100% + 6px)',zIndex:50,width:245,maxHeight:360,overflowY:'auto',background:'#fff',border:'1px solid #ddd',borderRadius:10,boxShadow:'0 10px 28px rgba(15,23,42,.15)',padding:8}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'5px 7px 8px',borderBottom:'1px solid #eee',marginBottom:4}}>
              <strong style={{fontSize:13}}>Content Type</strong>
              <button type="button" onClick={clearContentTypes} disabled={!selectedContentTypes.length} style={{border:0,background:'transparent',fontSize:12,fontWeight:600,cursor:selectedContentTypes.length?'pointer':'default',opacity:selectedContentTypes.length?1:.45}}>Clear</button>
            </div>
            {CONTENT_TYPES.map(type=><label key={type} style={{display:'flex',alignItems:'center',gap:9,padding:'8px 7px',borderRadius:7,cursor:'pointer',fontSize:13}}>
              <input type="checkbox" checked={selectedContentTypes.includes(type)} onChange={()=>toggleContentType(type)} />
              <span>{type}</span>
            </label>)}
          </div>}
        </div>

        <button className={`list-tool-btn ${filters.period!=='all'?'active':''}`} type="button" onClick={()=>setFilters(f=>({...f,period:f.period==='all'?'week':f.period==='week'?'month':'all'}))} title="Filter by week or month">{filters.period==='week'?'This Week':filters.period==='month'?'This Month':'Filters'}</button>
    ...
    <div style={{display:'flex',justifyContent:'flex-end',gap:6,marginBottom:10}}>
      {[['all','All'],['week','This Week'],['month','This Month']].map(([value,label])=><button key={value} type="button" className={filters.period===value?'active':''} onClick={()=>setFilters(f=>({...f,period:value}))} style={{padding:'6px 10px',borderRadius:8,border:'1px solid #ddd',background:filters.period===value?'#eeeaff':'#fff',fontWeight:600}}>{label}</button>)}
    </div>

    {deleteError&&<div className="card" style={{marginBottom:12,padding:'10px 14px',color:'#b42318',background:'#fff1f0'}}>{deleteError}</div>}

    {loadingSkeleton?<div className="posts-loading" aria-live="polite"/>:groupByDate?<div className="date-groups">
    ...
    {showCreateModal&&<div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget){setShowCreateModal(false);setEditingPost(null)}}}>
      <div className="modal create-post-modal" role="dialog" aria-modal="true">
        <PostForm onSaved={()=>{setShowCreateModal(false);setEditingPost(null);load()}} onCancel={()=>{setShowCreateModal(false);setEditingPost(null)}}/>
      </div>
    </div>}
  </div>;
}
