...
const wantsList=q=>/show|list|which|what|give|name|names|নাম|দেখাও|লিস্ট|তালিকা|কি\s*কি|কী\s*কী|কোন\s*কোন|কোনগুলো|কোন গুলো/.test(normalize(q));
const getChannel=q=>{const m=normalize(q).match(/(?:^|[^a-z])(hhd|bhd|dhd)(?:$|[^a-z])/i);return m?m[1].toUpperCase():null};
const isListedVideo=x=>{const s=fieldStatus(x);return s.length>0&&s.every(v=>v==="")};
const isRecorderVideo=x=>{const s=fieldStatus(x);return s.length>0&&s.every(v=>v==="record")};
const isRunningVideo=x=>!isListedVideo(x)&&!isRecorderVideo(x)&&fieldStatus(x).some(v=>v!=="");
const stageOf=x=>isListedVideo(x)?"Listed":isRecorderVideo(x)?"Recorder":"Running";
const isEditingDone=x=>fieldStatus(x).some(v=>v==="editing done"||v==="edited"||v==="edit done"||v.includes("editing"));
...
function parseDhakaDate(value){
  ...
  const m=text.replace("T"," ").match(/^(\d{4})-(\d{2})-(\d{2})[ ](\d{2}):(\d{2})(?::(\d{2}))?/);
...
function previousUser(history){return normalize(history?.slice().reverse().find(x=>String(x?.role||"")==="user")?.text||"")}
function followupKind(q,history){
  const l=normalize(q), prev=previousUser(history); if(!prev)return null;
  const hasFollowupWord=/^\s*(?:name|names|নাম|নামগুলো|নাম গুলো|নাম বল|নামগুলো বল|নাম বলো|কি\s*কি|কী\s*কী|কোন\s*কোন|কোনগুলো|কোন গুলো|which|what|show me|show list|list them|give me the list)\s*$/.test(l);
  const shortFollowup=/(নাম|names?|কি\s*কি|কী\s*কী|কোনগুলো|কোন গুলো|which|list)/.test(l)&&l.length<60;
  if(!hasFollowupWord&&!shortFollowup)return null;
  if(/edit|editing|এডিট|এডিটিং/.test(prev))return"editing";
  ...
  if(/schedule|scheduled|calendar|শিডিউল|ক্যালেন্ডার/.test(prev))return"scheduled";
  if(/post|posted|upload|আপলোড|পোস্ট/.test(prev))return"posts";
  return"general";
}

function inheritedChannel(q,history){return getChannel(q)||getChannel(previousUser(history))}
...
function videoAnswer(q,contents,history){
  ...
  const asksVideo=/video|content|ভিডিও|কনটেন্ট/.test(l)||!!follow;
  if(!asksVideo)return null;
  ...
  if(!channel&&!isCount(l)&&!wantsList(l))return null;
...
function scheduleAnswer(q,posts,history){
  const l=normalize(q), follow=followupKind(q,history);
  if(follow!=="scheduled"&&!/schedule|scheduled|calendar|শিডিউল|ক্যালেন্ডার|পোস্ট|today|tomorrow|আজ|আজকে|আজকের|আগামীকাল/.test(l))return null;
  const ch=inheritedChannel(q,history);
  const all=posts.filter(p=>statusOf(p)==="scheduled"&&p?.scheduled_at&&(!ch||channelOf(p)===ch));
  ...
  if((today||tomorrow)||when||wantsList(l)||follow==="scheduled"){
...
function postAnswer(q,posts,history){
  const l=normalize(q),ch=inheritedChannel(q,history),rows=posts.filter(p=>!ch||channelOf(p)===ch),follow=followupKind(q,history);
  if(follow==="posts"&&!/post|posted|upload|আপলোড|পোস্ট/.test(l))return`${ch?ch+" ":""}Posts: ${rows.length}\n\n${rows.slice(0,100).map((p,i)=>`${i+1}. ${p.project_name||"Untitled"} — ${p.status||"Not set"}`).join("\n")}`;
...
export async function handleChat(request,env){
  ...
  const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model:String(env.OPENAI_MODEL||"gpt-5.6-luna"),instructions:"You are the private data assistant for a Content Schedule Manager. Use ONLY the LIVE DATABASE DATA below. Never invent data. Understand Bangla, Banglish and English. Preserve conversation context from RECENT CONVERSATION. A short follow-up such as নাম, কি কি, কোনগুলো, when, which, list them refers to the previous user question and inherits its channel and intent unless the user changes them. Answer exact counts and matching records. Dates/times are Asia/Dhaka. Scheduled means status Scheduled with scheduled_at present. A Recorder video is a content row where all five video status fields are Record, matching the app's Content page. An Editing Done video is a content row where at least one of the five video status fields is Editing Done. You may answer questions about any field present in contents or posts, including names, channels, video status fields, poster status, project names, content types, platforms, post status, schedule dates/times and uploaded links. If the requested information is not present in LIVE DATA, say it is not available instead of guessing. Read-only assistant.",input:`CURRENT QUESTION:\n${q}\n\nRECENT CONVERSATION:\n${previous||"none"}\n\nLIVE DATABASE DATA:\n${JSON.stringify(ctx)}`,store:false,max_output_tokens:1800})});
