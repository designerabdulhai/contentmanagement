...
import { handleChatRoute } from './chatRouteFix.js';
...
async function writeAudit(db, postId, action, payload) {
  try {
    await db.prepare(`INSERT INTO audit (post_id, action, payload, actor) VALUES (?, ?, ?, ?)`).bind(
      postId ?? null,
      action,
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      console.error('Worker entry error:', error?.message || error);
      return json({
        ok: false,
