...
async function handle(request, env) {
  ...
  if (method === 'POST' && path === '/api/posts/bulk') {
    ...
    const days = Array.isArray(p.days_of_week)
      ? p.days_of_week.map(Number).filter(n => Number.isInteger(n) && n >= 0 && n <= 6)
      : [];
    ...
    while ((end ? cursor <= end : count < maxOccurrences) && count < maxOccurrences) {
      if (days.length === 0 || days.includes(cursor.getDay())) {
        statements.push(db.prepare(`INSERT INTO posts (project_name, content_type, channel, platform, status, scheduled_at, created_by)
          VALUES (?, ?, ?, ?, 'Scheduled', ?, ?)`)
    ...
    const created = await db.prepare(`
      SELECT * FROM posts WHERE project_name = ? ORDER BY id DESC LIMIT ?
    `).bind(p.project_name, count).all();
  ...
  if (method === 'POST' && path === '/api/templates') {
    ...
    const result = await db.prepare(`INSERT INTO templates (name, content_type, channel, platform, project_id, created_by)
      VALUES (?, ?, ?, ?, ?, ?)`).bind(p.name, p.content_type || null, p.channel || null, p.platform || null, p.project_id || null, p.created_by || null).run();
    const row = await db.prepare('SELECT * FROM templates WHERE id = ?').bind(result.meta.last_row_id).first();
    return json(row, 201);
  }
  ...
  if (notesMatch) {
    ...
    if (method === 'GET') {
      const result = await db.prepare(`SELECT pn.*, u.display_name AS user_name FROM post_notes pn
        LEFT JOIN users u ON pn.user_id = u.id WHERE pn.post_id = ? ORDER BY pn.created_at DESC`).bind(postId).all();
      return json(result.results || []);
    }
    if (method === 'POST') {
      const p = await body(request);
      if (!String(p.message || '').trim()) return json({ error: 'message required' }, 400);
      const post = await db.prepare('SELECT id FROM posts WHERE id = ?').bind(postId).first();
      if (!post) return json({ error: 'post not found' }, 404);
      const result = await db.prepare('INSERT INTO post_notes (post_id, user_id, message) VALUES (?, ?, ?)')
        .bind(postId, p.user_id || null, String(p.message).trim()).run();
      const row = await db.prepare(`SELECT pn.*, u.display_name AS user_name FROM post_notes pn
        LEFT JOIN users u ON pn.user_id = u.id WHERE pn.id = ?`).bind(result.meta.last_row_id).first();
      return json(row, 201);
    }
  }

  if (method === 'GET' && path === '/api/dashboard/due-soon') {
    const result = await db.prepare(`SELECT p.*, u.display_name AS owner FROM posts p LEFT JOIN users u ON p.created_by = u.id
      ...
      ORDER BY p.scheduled_at ASC LIMIT 50`).all();
    return json(result.results || []);
  }

  if (method === 'POST' && path === '/api/overdue/check') {
    const result = await db.prepare(`UPDATE posts SET is_overdue = CASE
      WHEN status = 'Scheduled' AND scheduled_at IS NOT NULL AND scheduled_at < datetime('now') THEN 1 ELSE 0 END`).run();
  ...
  if (method === 'POST' && path === '/api/posts') {
    ...
    const result = await db.prepare(`INSERT INTO posts (project_name, content_type, channel, platform, status, scheduled_at, uploaded_link, notes, created_by, recurring_rule)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
      p.project_name, p.content_type || null, p.channel || null, p.platform || null, p.status || 'Listed',
      p.scheduled_at || null, p.uploaded_link || null, p.notes || null, p.created_by || null, p.recurring_rule || null
    ).run();
    const row = await db.prepare('SELECT * FROM posts WHERE id = ?').bind(result.meta.last_row_id).first();
    await db.prepare('INSERT INTO audit(post_id, action, payload, actor) VALUES (?, ?, ?, ?)')
      .bind(row.id, 'create', JSON.stringify(row), p.created_by || 'system').run();
    return json(row, 201);
  }
  ...
  if (postMatch) {
    ...
    if (method === 'PUT') {
      ...
      await db.prepare(`UPDATE posts SET project_name=?, content_type=?, channel=?, platform=?, status=?, scheduled_at=?, uploaded_link=?, notes=?, recurring_rule=?, updated_at=datetime('now') WHERE id=?`)
        .bind(p.project_name || null, p.content_type || null, p.channel || null, p.platform || null, p.status || null, p.scheduled_at || null, p.uploaded_link || null, p.notes || null, p.recurring_rule || null, id).run();
      const row = await db.prepare('SELECT * FROM posts WHERE id = ?').bind(id).first();
      await db.prepare('INSERT INTO audit(post_id, action, payload, actor) VALUES (?, ?, ?, ?)')
        .bind(id, 'update', JSON.stringify(row), p.updated_by || 'system').run();
      return json(row);
    }
    if (method === 'DELETE') {
      const row = await db.prepare('SELECT * FROM posts WHERE id = ?').bind(id).first();
      ...
      const p = await body(request);
      await db.prepare('INSERT INTO audit(post_id, action, payload, actor) VALUES (?, ?, ?, ?)')
        .bind(id, 'delete', JSON.stringify(row), p.actor || 'system').run();
  ...
  if (method === 'POST' && duplicateMatch) {
    ...
    const p = await body(request);
    const result = await db.prepare(`INSERT INTO posts (project_name, content_type, channel, platform, status, scheduled_at, uploaded_link, notes, created_by, recurring_rule)
      VALUES (?, ?, ?, ?, 'Listed', NULL, NULL, ?, ?, ?)`).bind(
      row.project_name, row.content_type, row.channel, row.platform, row.notes, p.created_by || row.created_by, row.recurring_rule
    ).run();
  ...
  if (method === 'POST' && uploadedMatch) {
    const id = Number(uploadedMatch[1]);
    const exists = await db.prepare('SELECT id FROM posts WHERE id = ?').bind(id).first();
    if (!exists) return json({ error: 'post not found' }, 404);
    const p = await body(request);
    await db.prepare("UPDATE posts SET status='Uploaded', uploaded_link=?, is_overdue=0, updated_at=datetime('now') WHERE id=?")
      .bind(p.uploaded_link || null, id).run();
  ...
  if (method === 'GET' && path === '/api/settings') {
    ...
    return json(settingObject(result.results || []));
  }

  const settingMatch = path.match(/^\/api\/settings\/([a-zA-Z0-9_-]+)$/);
  if (method === 'PUT' && settingMatch) {
    const p = await body(request);
    const values = Array.isArray(p) ? p.map(v => String(v).trim()).filter(Boolean) : [];
  ...
  if (method === 'GET' && path === '/api/invites') {
    const result = await db.prepare('SELECT id, email, role, status, created_at FROM invites ORDER BY created_at DESC').all();
    return json(result.results || []);
  }
